...
def run_trial(win, trial, config, fixation, trial_clock, extra_text, clock_image, timer, show_feedback, feedback, block_type, recall_info, mouse):
    answer = None
    reaction_time = None
    acc = -1
    ...
    while trial_clock.getTime() < config["matrix_2_time"] and not answer:
        trial.matrix_2.draw()
        show_clock(clock_image, trial_clock, config)
        show_timer(timer, trial_clock, config)
        draw_recall_point(recall_info)

        for file_name, stimulus in zip(trial.matrix_2.stimulus_list, trial.matrix_2.stimulus_to_draw):
            if mouse.isPressedIn(stimulus["stim_to_draw"]):
    ...
    if answer:
        acc = int(answer == trial.changed_element)
